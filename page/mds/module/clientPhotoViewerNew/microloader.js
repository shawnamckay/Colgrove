var Ext = Ext || { }; Ext.manifest = Ext.manifest || "mds/module/clientPhotoViewerNew/app.json";// @tag core
// @define Ext.Boot

//CUSTOM CHANGES:
//in function Entry(cfg) {
// mvCacheExclude check,
// add in line 1276
// if (cfg.url.match(/ext-[\d\.]*\.js/)) buster=undefined; //Cache framework
...
Ext.Boot = Ext.Boot || (function (emptyFn) {
    ...
    function Entry(cfg) {
        ...
        if (Boot.config.disableCaching || window.mvCacheExclude) {
            if (cache === undefined) {
                cache = !Boot.config.disableCaching;
            }

            if (window.mvCacheExclude) {
                for (var i = 0; i < window.mvCacheExclude.length; i++) {
                    if (cfg.url.match(window.mvCacheExclude[i]))
                        cache = false;
                }
            }

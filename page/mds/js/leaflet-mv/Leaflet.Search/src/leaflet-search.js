...
MVLeaflet.Control.Search = MVLeaflet.Control.extend({
	...
	//
	//Public methods:
	//  setLayer()				MVLeaflet.LayerGroup()      set layer search at runtime
	//  showAlert()             'Text message'      Show alert message
	//
	options: {
		url: '',					//url for search by ajax request, ex: "search.php?q={s}"
		jsonpParam: null,			//jsonp param name for search by jsonp service, ex: "callback"
		layer: null,				//layer where search markers(is a MVLeaflet.LayerGroup)		
...
//FIXME option condition problem {autoCollapse: false }
	...
	initialize: function(options) {
		...
		this._layer = this.options.layer || new MVLeaflet.LayerGroup();
	...
	onAdd: function (map) {
		this._map = map;
		this._container = MVLeaflet.DomUtil.create('div', 'leaflet-control-search');
	...
	// 	if(e.layer instanceof MVLeaflet.LayerGroup)
	// 		if( MVLeaflet.stamp(e.layer) != MVLeaflet.stamp(this._layer) )
	...
	expand: function() {		
		...
		MVLeaflet.DomUtil.addClass(this._container, 'search-exp');	
		this._input.focus();
	...
	collapse: function() {
		...
		this._input.blur();
		this._cancel.style.display = 'none';
		MVLeaflet.DomUtil.removeClass(this._container, 'search-exp');		
...
////start DOM creations
	_createAlert: function(className) {
		var alert = MVLeaflet.DomUtil.create('div', className, this._container);
		alert.style.display = 'none';

		MVLeaflet.DomEvent
			.on(alert, 'click', MVLeaflet.DomEvent.stop, this)
	...
	_createInput: function (text, className) {
		var input = MVLeaflet.DomUtil.create('input', className, this._container);
		input.type = 'text';
	...
	_createCancel: function (title, className) {
		var cancel = MVLeaflet.DomUtil.create('a', className, this._container);
		cancel.href = '#';
		cancel.title = title;
		...
		MVLeaflet.DomEvent
			.on(cancel, 'click', MVLeaflet.DomEvent.stop, this)
	...
	_createButton: function (title, className) {
		var button = MVLeaflet.DomUtil.create('a', className, this._container);
		button.href = '#';
		button.title = title;

		MVLeaflet.DomEvent
			.on(button, 'click', MVLeaflet.DomEvent.stop, this)
	...
	_createTooltip: function(className) {
		var tool = MVLeaflet.DomUtil.create('div', className, this._container);
		...
		MVLeaflet.DomEvent
			...
			.on(tool, 'mousewheel', function(e) {
				...
				MVLeaflet.DomEvent.stopPropagation(e);//disable zoom map
			}, this)
	...
	_createTip: function(text, val) {//val is object in recordCache, usually is Latlng
		...
		if(this.options.callTip)
		{
			...
			{
				var tmpNode = MVLeaflet.DomUtil.create('div');
		...
		{
			tip = MVLeaflet.DomUtil.create('a', '');
			tip.href = '#';
			tip.innerHTML = text;
		}
		
		MVLeaflet.DomUtil.addClass(tip, 'search-tip');
		tip._text = text; //value replaced in this._input and used by _autoType

		MVLeaflet.DomEvent
			.disableClickPropagation(tip)		
			.on(tip, 'click', MVLeaflet.DomEvent.stop, this)
...
//////end DOM creations
	...
	_defaultFilterJSON: function(json) {	//default callback for filter data
		...
		if( MVLeaflet.Util.isArray(propLoc) )
			for(var i in json)
				jsonret[ json[i][propName] ]= MVLeaflet.latLng( json[i][ propLoc[0] ], json[i][ propLoc[1] ] );
		else
			for(var n in json)
				jsonret[ json[n][propName] ]= MVLeaflet.latLng( json[n][ propLoc ] );
	...
	_recordsFromJsonp: function(text, callAfter) {  //extract searched records from remote jsonp service
		...
		var that = this;
		MVLeaflet.Control.Search.callJsonp = function(data) {	//jsonp callback
			var fdata = that._filterJSON(data);//_filterJSON defined in inizialize...
			callAfter(fdata);
		}
		var script = MVLeaflet.DomUtil.create('script','search-jsonp', document.getElementsByTagName('body')[0] ),			
			url = MVLeaflet.Util.template(this.options.url+'&'+this.options.jsonpParam+'=MVLeaflet.Control.Search.callJsonp', {s: text}); //parsing url
	...
	_recordsFromAjax: function(text, callAfter) {	//Ajax request
		...
		var request = new XMLHttpRequest(),
			url = MVLeaflet.Util.template(this.options.url, {s: text}), //parsing url
			//rnd = '&_='+Math.floor(Math.random()*10000);
	...
	_recordsFromLayer: function() {	//return table: key,value from layer
		...
		this._layer.eachLayer(function(layer) {
			...
			if(layer instanceof MVLeaflet.Marker)
			{
...
//	in this mode every record can have a free structure of attributes, only 'loc' is required
		...
		MVLeaflet.DomUtil.addClass(this._container, 'search-load');
		...
		if(this.options.callData)	//CUSTOM SEARCH CALLBACK(USUALLY FOR AJAX SEARCHING)
		{
			...
			this.options.callData(inputText, function(jsonraw) {
				...
				MVLeaflet.DomUtil.removeClass(that._container, 'search-load');
			});
		}
		...
		else if(this.options.layer)	//SEARCH ELEMENTS IN PRELOADED LAYER
		{
			...
			this.showTooltip();
			MVLeaflet.DomUtil.removeClass(this._container, 'search-load');
		}
	},
	...
	_handleAutoresize: function() {	//autoresize this._input
	    ...
	    if (this._input.style.maxWidth != this._map._container.offsetWidth) //If maxWidth isn't the same as when first set, reset to current Map width
	        this._input.style.maxWidth = MVLeaflet.DomUtil.getStyle(this._map._container, 'width');
	...
	_handleArrowSelect: function(velocity) {
		...
		for (i=0; i<searchTips.length; i++)
			MVLeaflet.DomUtil.removeClass(searchTips[i], 'search-tip-select');
		
		if ((velocity == 1 ) && (this._tooltip.currentSelection >= (searchTips.length - 1))) {// If at end of list.
			MVLeaflet.DomUtil.addClass(searchTips[this._tooltip.currentSelection], 'search-tip-select');
		}
...
var SearchMarker = MVLeaflet.Marker.extend({
	...
	initialize: function (latlng, options) {
		...
		MVLeaflet.Marker.prototype.initialize.call(this, latlng, options);
		this._circleLoc = new MVLeaflet.CircleMarker(latlng, this.options);
		//TODO add inner circle
	},

	onAdd: function (map) {
		MVLeaflet.Marker.prototype.onAdd.call(this, map);
		map.addLayer(this._circleLoc);
		this.hide();
	},

	onRemove: function (map) {
		MVLeaflet.Marker.prototype.onRemove.call(this, map);
		map.removeLayer(this._circleLoc);
	},	
	
	setLatLng: function (latlng) {
		MVLeaflet.Marker.prototype.setLatLng.call(this, latlng);
...
MVLeaflet.Map.addInitHook(function () {
    if (this.options.searchControl) {
        this.searchControl = MVLeaflet.control.search(this.options.searchControl);
        this.addControl(this.searchControl);
    }
});

MVLeaflet.control.search = function (options) {
    return new MVLeaflet.Control.Search(options);
};
